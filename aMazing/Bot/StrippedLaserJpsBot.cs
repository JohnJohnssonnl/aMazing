using System;
using System.Collections.Generic;

public class StrippedLaserJpsBot : BaseMazeSolver
{
    public override string Name => "Stripped Laser JPS Bot";

    private class JpsNode
    {
        public int X { get; }
        public int Y { get; }
        public int G { get; set; }
        public int H { get; }
        public int F => G + H;
        public JpsNode Parent { get; set; }

        public JpsNode(int x, int y, int g, int h, JpsNode parent)
        {
            X = x; Y = y; G = g; H = h; Parent = parent;
        }
    }

    private int[,] _gScores;
    private bool[,] _closed;

    public override List<(int x, int y)> Solve(char[,] maze, (int x, int y) start, (int x, int y) end)
    {
        int height = maze.GetLength(0);
        int width = maze.GetLength(1);

        if (_gScores == null || _gScores.GetLength(0) != height || _gScores.GetLength(1) != width)
        {
            _gScores = new int[height, width];
            _closed = new bool[height, width];
        }
        else
        {
            Array.Clear(_gScores, 0, _gScores.Length);
            Array.Clear(_closed, 0, _closed.Length);
        }

        var openQueue = new PriorityQueue<JpsNode, int>();

        int startH = Math.Abs(start.x - end.x) + Math.Abs(start.y - end.y);
        var startNode = new JpsNode(start.x, start.y, 0, startH, null);
        openQueue.Enqueue(startNode, startNode.F);

        int maxX = width - 1;
        int maxY = height - 1;

        int[] dx = { 0, 0, -1, 1 };
        int[] dy = { -1, 1, 0, 0 };

        while (openQueue.Count > 0)
        {
            JpsNode curr = openQueue.Dequeue();

            if (_closed[curr.Y, curr.X]) continue;
            _closed[curr.Y, curr.X] = true;

            if (curr.X == end.x && curr.Y == end.y)
            {
                return ReconstructPath(curr);
            }

            for (int i = 0; i < 4; i++)
            {
                int x = curr.X;
                int y = curr.Y;
                int stepX = dx[i];
                int stepY = dy[i];

                while (true)
                {
                    x += stepX;
                    y += stepY;

                    if (x <= 0 || x >= maxX || y <= 0 || y >= maxY || maze[y, x] == '#')
                        break;

                    if (x == end.x && y == end.y)
                    {
                        PushNode(curr, x, y, end, openQueue);
                        break;
                    }

                    if (stepX != 0)
                    {
                        if ((maze[y + 1, x] != '#' && maze[y + 1, x - stepX] == '#') ||
                            (maze[y - 1, x] != '#' && maze[y - 1, x - stepX] == '#'))
                        {
                            PushNode(curr, x, y, end, openQueue);
                            break;
                        }
                    }
                    else
                    {
                        if ((maze[y, x + 1] != '#' && maze[y - stepY, x + 1] == '#') ||
                            (maze[y, x - 1] != '#' && maze[y - stepY, x - 1] == '#'))
                        {
                            PushNode(curr, x, y, end, openQueue);
                            break;
                        }
                    }
                }
            }

            LeaveBreadcrumb(curr.X, curr.Y);
        }

        return null;
    }

    private void PushNode(JpsNode curr, int jx, int jy, (int x, int y) end, PriorityQueue<JpsNode, int> openQueue)
    {
        if (_closed[jy, jx]) return;

        int newG = curr.G + (Math.Abs(curr.X - jx) + Math.Abs(curr.Y - jy));
        int existingG = _gScores[jy, jx];

        if (existingG == 0 || newG < existingG)
        {
            _gScores[jy, jx] = newG;
            int newH = Math.Abs(jx - end.x) + Math.Abs(jy - end.y);
            var node = new JpsNode(jx, jy, newG, newH, curr);
            openQueue.Enqueue(node, node.F);
        }
    }

    private List<(int x, int y)> ReconstructPath(JpsNode node)
    {
        List<(int, int)> path = new List<(int, int)>();
        JpsNode temp = node;
        while (temp != null)
        {
            path.Add((temp.X, temp.Y));
            temp = temp.Parent;
        }
        path.Reverse();

        List<(int, int)> fullPath = new List<(int, int)>();
        for (int i = 0; i < path.Count - 1; i++)
        {
            var p1 = path[i];
            var p2 = path[i + 1];
            int x = p1.Item1;
            int y = p1.Item2;

            while (x != p2.Item1 || y != p2.Item2)
            {
                fullPath.Add((x, y));
                x += Math.Sign(p2.Item1 - x);
                y += Math.Sign(p2.Item2 - y);
            }
        }
        fullPath.Add(path[path.Count - 1]);
        return fullPath;
    }
}